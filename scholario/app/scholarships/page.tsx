...
function isActive(deadline: string | undefined): boolean {
  if (!deadline || deadline === "—") return true;
  const lower = deadline.toLowerCase();
  if (["rolling", "ongoing", "open", "tbd", "flexible"].some((k) => lower.includes(k))) return true;
...
function ScholarshipCard({ s }: { s: Scholarship }) {
  const active = isActive(s.deadline);
  return (
    <div className={`bg-white rounded-2xl p-6 border transition-all flex flex-col ${active ? "border-slate-200 hover:shadow-lg hover:border-slate-300" : "border-slate-100 opacity-60"}`}>
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          <span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${active ? "bg-emerald-50 text-emerald-600" : "bg-red-50 text-red-500"}`}>
...
export default function ScholarshipsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return all.filter((s) => {
      ...
      const matchesDegree =
        filterDegree === "All" || s.degreeLevel === filterDegree;
      ...
      return matchesSearch && matchesDegree && matchesFunding && matchesCountry && matchesStatus;
    });
  }, [all, search, filterDegree, filterFunding, filterCountry, filterStatus]);

  const bachelors = filtered.filter((s) =>
    s.degreeLevel?.toLowerCase().includes("bachelor") ||
    s.degreeLevel?.toLowerCase().includes("undergraduate")
  );
  const masters = filtered.filter((s) =>
    s.degreeLevel?.toLowerCase().includes("master") ||
    s.degreeLevel?.toLowerCase().includes("postgraduate") ||
    s.degreeLevel?.toLowerCase() === "msc" ||
    s.degreeLevel?.toLowerCase() === "mba"
  );
  ...
  const selectClass =
    "px-3 py-2 rounded-xl border border-slate-200 bg-white text-slate-700 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500";
